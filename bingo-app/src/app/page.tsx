...
export default function BingoPage() {
  ...
  // If the user changes the auto-call delay while running, restart the timer
  ...
  // The last few calls (excluding current)
  const previousCalls = calledNumbers.slice(-5, -1);

  // Renders B-1, I-25, etc.
  const formatCell = (num: number) => `${getBingoLetter(num)}-${num}`;

  return (
    <div className={styles.pageContainer}>
      {/* Top bar: recent calls (left), big current call (center), gear (right) */}
        ...
        {showSettings && (
          <div className={styles.settingsPanel}>
            ...
            <div className={styles.settingsButtons}>
              ...
              <button onClick={callNextNumber}>Next</button>
            </div>
          </div>
        )}
      </div>

      {/* The 5x15 board, no scrolling. Each cell is large. */}
          ...
          [...Array(15)].map((_, colIndex) => {
            ...
            return (
              <div
                key={n}
                className={
                  isCalled ? styles.boardCellCalled : styles.boardCell
                }
              >
                {formatCell(n)}
              </div>
